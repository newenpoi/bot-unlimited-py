...
class Dig(Cog):
    ...
    async def dig(self, interaction: Interaction) -> None:
        ...
        embed = Embed(title = f"\💎・{item.name}", description = (item.description or await reader.read(f'commands/dig.{language.country_code}', 'null', item.value, rarity, 9)), color = helper.raritycolor(item.rarity.id))
