...
class Lift(Cog):
    ...
    async def lift(self, interaction: Interaction, member: Member = SlashOption(required = True, description = "Qui souhaites-tu soulever ?")) -> None:
        ...
        n = len(user_service.find_user_interactions_having(interaction.user.id, 'Soulever'))
        if (n >= 3): response = await reader.read('commands/lift', 'max')
        
        # On a assez de vitalité pour soulever la personne.
        health = user_service.find_user_health(interaction.user.id)
        ...
        health = user_service.find_user_health(member.id)
        ...
        if (response): await interaction.send(response)
            ...
            health = random.randint(0, 9)

            # On ajuste les points de vie de la cible.
            user_service.update_health(member.id, -health)

            # On ajoute une interaction supplémentaire.
            user_service.add_interaction(interaction.user.id, 'Soulever')
            ...
            embed.set_image(url = f"http://18.168.128.213/img/hatada/lift/0{(random.randint(1, 7) if health > 0 else 0)}.gif")
